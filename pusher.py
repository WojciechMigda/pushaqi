...
# PM2.5 breakpoints for the US AQI (EPA 24-hour table, as used by the
# AIRLY_US_AQI widget index). Each entry's hi value is the exact upper
# bound of the band: 12.0 / 35.4 / 55.4 / 150.4 / 250.4 / 500.4.
AQI_PM25_LEVELS = {
    'Good' : dict(hi=12.0,
                  img='pics/00E400.gif',
                  sensitive=None,
                  regular=None),
    'Moderate': dict(hi=35.4,
                     img='pics/FFFF00.gif',
...
def coerce_former_aqi(value: Union[int, bool, None]) -> Union[bool, None]:
    """Normalize the ``former_aqi`` positional argument.

    ``None`` means "state unknown" (first run / missing aqi_flag.txt) and is
    preserved as-is; anything else is coerced to bool so the state machine in
    :func:`push_aqi_status` can distinguish *unknown* from *previously good*.
    """
    return None if value is None else bool(value)


CHAR_TEST_VECTORS: List[Tuple[str, str]] = [
...
def test_number_recognition():
    for path, number in NUMBER_TEST_VECTORS:
...
def pull_measurements(retries: int, timeout: int) -> Dict[str, Dict[str, str]]:
    ...
    def download(url: str):
        try:
            ...
            logging.error(f"Request failure: {e}")
            return None
    ...
    for sensor, url in SENSORS.items():
        ...
        maybe_address: etree._Element = root.find('.//td[@class="summary__address"]')
...
def to_float(value: Union[str, int, float, None]) -> Union[float, None]:
    """Best-effort conversion of a scraped widget value to float.

    The SVG decoder only knows the glyphs in CHAR_MAP (digits and minus),
    so a value extracted from the widget may carry trailing junk — e.g. a
    decimal-point glyph or a unit symbol that has no signature. Returning
    None lets callers skip a sensor instead of crashing the whole run.
    """
    if value is None:
        return None
    ...
    try:
        with Path(path).open('rb') as fileobj:
            ...
            res: requests.Response = session.post(url, timeout=timeout,
                ...
                headers=dict(Authorization=f'Bearer {MASTODON_TOKEN}'))
        ...
        # e.g. the AQI image file is missing from the checkout
        logging.error(f"Local file failure: {e}")
    return False, None
...
def push_aqi_status(
    ...
    for sensor, data in measurements.items():
        value = to_float(data.get('pm2.5')) if 'pm2.5' in data else None
        if value is None:
            logging.warning(f'Sensor {sensor}: unparseable PM2.5 value {data.get("pm2.5")!r}, skipping.')
            continue
        pm25.append(value)
    ...
    if send_flag:
        ...
        if ok:
            ...
            # Keep the previous state (aqi_flag.txt / aqi_status.txt untouched)
            # so the next run retries the failed transition.
            logging.error('Status POST failed; state files left unchanged for retry.')
            return
...
def main(
    ...
    # Keep the None sentinel: None means "state unknown" (first run / missing
    # aqi_flag.txt) and must not be coerced to False, or the first-run post
    # branch in push_aqi_status becomes dead code.
    former_aqi = coerce_former_aqi(former_aqi)
